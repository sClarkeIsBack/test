...
##########################################
def catchup():
    loginurl   = "http://flawless-iptv.net:4545/get.php?username=" + username + "&password=" + password + "&type=m3u_plus&output=ts"
    try:
    ...
    url = base64.b64decode("JXM6JXMveG1sdHYucGhwP3VzZXJuYW1lPSVzJnBhc3N3b3JkPSVz")%(host,port,username,password)
...
def addonsettings(url,description):
	if   url =="CC":
		...
		dialog = xbmcgui.Dialog().select('Select a TV Guide to Setup', ['Enable 1GB Ram or Lower AS','Enable 2GB Ram or Higher AS','Disable AS'])
		if dialog==0:
			lessthan()
			xbmcgui.Dialog().ok('MediaHub IPTV', 'Advanced Settings Set')
		elif dialog==1:
			morethan()
			xbmcgui.Dialog().ok('MediaHub IPTV', 'Advanced Settings Set')
		elif dialog==2:
